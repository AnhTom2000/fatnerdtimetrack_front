<template>
  <div class="watch">
    <header class="header">
      ...
      <div class="icon-priority" style="float:right" @click.stop.prevent="show_priority">
        <svg
          t="1587029230955"
          class="icon"
          viewBox="0 0 1024 1024"
      ...
      <div class="select-priority" v-if="priority_show" v-clickoutside="handleClosePriority">
        <h3>优先级</h3>
        <div class="priority-table">
          <ul>
            <li
              :class="{'active' : current_item.priorityId === index+1}"
      ...
      <div class="block" style="float:left;margin-left:90px">
        <el-date-picker
          v-model="current_item.date"
          type="datetime"
          placeholder="选择日期时间"
          size="small"
    ...
    <section class="item_main">
      <div class="caption-section">
        <ft-area :placeholder="'要做点什么'" :value.sync="current_item.eventTitle"></ft-area>
      </div>
      <div class="description">
        <ft-area
          :placeholder="'添加描述'"
          :value.sync="current_item.eventDescription"
    ...
    <footer class="footer">
      <div class="tag-left">
        <el-tag
          :key="tag.tagId"
          v-for="tag in current_item.tagList"
          closable
          :disable-transitions="false"
          @close="handleClose(tag)"
          :color="tag.tagBgColor"
        >{{tag.tagName}}</el-tag>
        <el-link
          type="primary"
          :underline="false"
          style="margin-left:10px"
          @click.stop.prevent="tag_show = !tag_show"
        >添加标签</el-link>
      </div>
      <div class="select-tag" v-if="tag_show" v-clickoutside="handleCloseTag">
        <div class="tag-input">
          <el-input
            class="input-new-tag"
      ...
      <div class="Trash-right" @click="deleteThisItem">
...
<script>
import ftArea from "../components/Ft-textarea";
...
export default {
  ...
  methods: {
    ...
    initList() {
      ...
      var tags = this.current_item.tagList;
      // console.log(tags);
    ...
    selected_priority(index) {
      this.showPropress();
      this.current_item.priorityId = index + 1;
      this.$store.dispatch("updateEventPriority", {
        priorityId: this.current_item.priorityId,
        eventId: this.current_item.eventId
      });
      this.priority_show = !this.priority_show;
    },
    handleClose(tag) {
      ...
      this.$store.dispatch("deleteEventTag", {
        tag: tag,
        event: this.current_item
      });
      this.current_item.tagList.splice(
        this.current_item.tagList.indexOf(tag),
        1
      );
      this.getTagList();
      this.initList();
    ...
    deleteThisItem() {
      ...
      this.$confirm("此操作将永久删除该事件, 是否继续?", "删除事件", {
        ...
        .then(() => {
          this.showPropress();
          this.$store.dispatch("deleteEvent", this.current_item);
        })
        .catch(() => {
          this.$message({
            type: "success",
    ...
    "current_item.eventTitle": {
      handler(value, old) {
        ...
        if (value != undefined) {
          ...
          vm.$store.dispatch("updateEventTitle", {
            title: value,
            event: vm.current_item
          });
        }
      },
      immediate: true,
    ...
    "current_item.date": {
      handler(value, old) {
        ...
        if (value != undefined) {
          ...
          vm.$store.dispatch("updateEventDate", vm.current_item);
        }
      },
      immediate: true,
      deep: true
    },
    "current_item.eventDescription": {
      handler(value, old) {
        var vm = this;
        if (value != undefined) {
          setTimeout(() => {
          ...
          vm.$store.dispatch("updateEventDescription", vm.current_item);
        }
      },
      immediate: true,
      deep: true
...
.watch .footer .tag-left {
  float: left;
  width: 600px;
}

.watch .footer .Trash-right {
  float: right;
  cursor: pointer;
}
...
.watch .tag-list li:hover {
  background-color: rgba(236, 239, 245, 0.9);
}
.watch .tag-list ul li span {
  position: absolute;
  top: 0;
  left: 0;
}

.watch .tag-list ul li i {
  display: inline-block;
  margin: 7px 10px;
}
.watch .select-tag .tag-btn {
  width: 200px;
  height: 40px;
  padding: 0px 10px;
  resize: none;
}
.watch .select-tag .tag-btn .el-button {
  margin: 0 15px !important;
}
.watch .el-input--suffix .el-input__inner {
  padding-right: 15px;
}
</style>